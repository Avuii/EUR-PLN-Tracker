...
# ŚCIEŻKI
# =========================
PROJECT_ROOT = Path(__file__).resolve().parents[1]  # <-- EurPlnTracker/
DATA_PATH = PROJECT_ROOT / "data" / "eur_a.csv"
...
# =========================
def main():
    global TEST_SIZE_SAMPLES, ZOOM_WINDOW_DAYS, FORECAST_HORIZON_DAYS
    ap = argparse.ArgumentParser()
    ap.add_argument("--no-tuning", action="store_true")

    ap.add_argument("--forecast-days-7", type=int, default=7)
    ap.add_argument("--forecast-days-1m", type=int, default=22)  # ~1 miesiąc (dni robocze)
    ap.add_argument("--forecast-days-12m", type=int, default=260)  # ~12 miesięcy (dni robocze)
    ...
    ZOOM_WINDOW_DAYS = args.zoom_window_days
    ...
    # =========================
    # Forecast next 7 dni roboczych (proste: rekurencyjnie ridge/rf na poziomie)
    # Uwaga: to jest "recursive forecast" – wystarczy do UI na zaliczenie.
    # =========================
    future_dates = next_business_days(pd.to_datetime(feat["date"].iloc[-1]), FORECAST_HORIZON_DAYS)

    # do rekurencji bierzemy ostatnie wartości (poziomy) i dokładamy predykcje
    ...
    forecast_rows = []
    # ridge i rf osobno (żeby mieć 2 kolumny prognoz)
    hist_ridge = history_values.copy()
    hist_rf = history_values.copy()
    ...
    pd.DataFrame(forecast_rows).to_csv(OUT_DIR / "forecast_next7.csv", index=False)

    run_recursive_forecast(args.forecast_days_7, OUT_DIR / "forecast_next7.csv")
    run_recursive_forecast(args.forecast_days_1m, OUT_DIR / "forecast_next1m.csv")
    run_recursive_forecast(args.forecast_days_12m, OUT_DIR / "forecast_next12m.csv")
    ...
    plot_series(
        ...
        title=f"EUR — predykcje na teście (ostatnie {TEST_SIZE_SAMPLES} próbek)",
