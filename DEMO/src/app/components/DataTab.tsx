...
export default function DataTab() {
  ...
  const handleExportCsv = () => {
    const csv = ["date,value", ...rows.map((row) => `${row.date},${row.value}`)].join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "eurpln_demo_data.csv";
    a.click();
      ...
      <Card className="bg-gradient-to-br from-white/5 to-white/[0.02] border-white/10 p-5 rounded-2xl backdrop-blur-xl shadow-xl">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">Tabela kursów z mockowanego demo GitHub Pages</p>
          </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((row, index) => (
                  <TableRow key={index} className="border-white/5 hover:bg-white/5 transition-colors">
                    <TableCell className="font-mono text-sm text-white">{row.date}</TableCell>
